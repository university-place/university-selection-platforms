...
export default function UniversityPlacementsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'DRAFT':
        return <span className="px-2 py-1 text-xs font-medium rounded-full bg-gray-100 text-foreground flex items-center gap-1"><Clock className="w-3 h-3" /> Draft</span>;
      default:
    ...
    <DashboardLayout title="Student Placements" navLinks={navLinks} theme="green">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="pl-9 pr-4 py-2 border border-border rounded-lg w-64 text-sm"
          />
        </div>
      </div>
      ...
      <div className="bg-card rounded-xl shadow-sm border overflow-hidden">
        <div className="px-6 py-4 border-b">
          <h2 className="text-lg font-bold text-foreground">Accepted Students</h2>
          <p className="text-sm text-muted-foreground">Students who passed interview/exam and received acceptance</p>
        </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase">Deadline</th>
              ...
              {filteredStudents.length === 0 ? (
                ...
                filteredStudents.map((student) => (
                  <tr key={student.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-6 py-4">
                      <div>
                        <p className="text-sm font-medium text-foreground">{student.programName}</p>
                        <p className="text-xs text-muted-foreground">{student.programCode}</p>
                      </div>
                    </td>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-card rounded-xl w-full max-w-2xl max-h-[90vh] overflow-auto">
            <div className="sticky top-0 bg-card border-b p-4 flex justify-between items-center">
              ...
              <button onClick={() => setShowDetailsModal(false)} className="text-muted-foreground hover:text-muted-foreground">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="bg-blue-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div><span className="text-muted-foreground">Exam ID:</span> <span className="font-medium">{selectedStudent.examID}</span></div>
                  <div><span className="text-muted-foreground">Name:</span> <span className="font-medium">{selectedStudent.firstName} {selectedStudent.lastName}</span></div>
                  <div><span className="text-muted-foreground">Email:</span> <span className="font-medium">{selectedStudent.email}</span></div>
                  <div><span className="text-muted-foreground">Phone:</span> <span className="font-medium">{selectedStudent.phone}</span></div>
                  <div><span className="text-muted-foreground">Region:</span> <span className="font-medium">{selectedStudent.region}</span></div>
                  <div><span className="text-muted-foreground">Program:</span> <span className="font-medium">{selectedStudent.programName}</span></div>
                </div>
              </div>
              ...
              <div className="bg-purple-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div><span className="text-muted-foreground">Type:</span> <span className="font-medium">{selectedStudent.interviewType}</span></div>
                  <div><span className="text-muted-foreground">Date:</span> <span className="font-medium">{new Date(selectedStudent.interviewDate).toLocaleString()}</span></div>
                  <div><span className="text-muted-foreground">Result:</span> <span className="font-medium text-green-600">{selectedStudent.result}</span></div>
                  <div><span className="text-muted-foreground">Notes:</span> <span className="font-medium">{selectedStudent.resultNotes || '-'}</span></div>
                </div>
              </div>
              ...
              <div className="bg-green-50 p-4 rounded-lg">
                ...
                <div className="space-y-2 text-sm">
                  <p><span className="text-muted-foreground">Acceptance Message:</span></p>
                  <p className="bg-card p-3 rounded border border-green-200">{selectedStudent.acceptanceMessage || 'Congratulations! You have been accepted.'}</p>
                  
                  <div className="grid grid-cols-2 gap-3 mt-3">
                    <div><span className="text-muted-foreground">Confirmation Deadline:</span> <span className="font-medium">{new Date(selectedStudent.confirmationDeadline).toLocaleDateString()}</span></div>
                    <div><span className="text-muted-foreground">Status:</span> {getStatusBadge(selectedStudent.status)}</div>
                    {selectedStudent.confirmedAt && (
                      <div><span className="text-muted-foreground">Confirmed On:</span> <span className="font-medium">{new Date(selectedStudent.confirmedAt).toLocaleDateString()}</span></div>
                    )}
                  </div>
                </div>
              </div>
