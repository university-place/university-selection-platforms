...
// ============================================
// AUTHENTICATION UTILITY - WITH BYPASS
// ============================================
...
async function verifyStudent(request: Request) {
  ...
  // ============================================
  // ADD THIS BYPASS - For testing without token
  // ============================================
  if (!authHeader?.startsWith('Bearer ')) {
    const firstStudent = await prisma.student.findFirst()
    if (firstStudent) {
      console.log('⚠️ No token, using student ID:', firstStudent.id)
      return { studentId: firstStudent.id }
    }
    throw new Error('No token provided and no student found')
  }
  // ============================================
  ...
  try {
    ...
    // ============================================
    // ADD THIS BYPASS - For invalid token
    // ============================================
    console.log('⚠️ Invalid token, using first student')
    const firstStudent = await prisma.student.findFirst()
    if (firstStudent) {
      return { studentId: firstStudent.id }
    }
...
// (Keep all your helper functions, subjectDefinitions, calculateAge, etc.)
// ============================================
// HELPER FUNCTIONS
// ============================================
